    ...
    public function addLiveSession(Request $request, $id)
    {
        ...
        if (!empty($ReserveMeeting)) {
            ...
            if (!empty($session)) {
               ...
               // Generate the link for joining the session
                $joinLink = $session->getJoinLink();// url("/panel/sessions/{$session->id}/joinToAgora");

                // Update the session with the generated link
                $session->update([
                    'link' => $joinLink,
                ]);
